...
import requests
import json
from brownie import AdvancedCollectible, network, accounts, TokenERC20, config
from metadata import sample_metadata
from scripts.helpful_scripts import get_address, get_totalSupply, get_decimals, get_drought, get_fire, get_flood, get_geo, get_soil, get_trees, get_type
...
breed_to_image_uri = {
    ...
    "LAND_CONSERVATION":"ipfs://QmS5jCAwiMFwvQ2ugdVMrsZVEuw9PubzhCPYcGgVQRXRhA?filename=land-conservation.png"
}
...
def write_metadata(token_ids, nft_contract):
    for token_id in range(token_ids):
        ...
        if Path(metadata_file_name).exists():
            ...
            collectible_metadata["token_address"] = get_address(erc20)
            
            collectible_metadata["token_total_supply"] = get_totalSupply(erc20)
            
            collectible_metadata["token_decimals"] = get_decimals(erc20)
